exports.placedOrder = (data) => {
    ...
    <p>Jeżeli wybrałeś formę płatności Przelew tradycyjny, to proszę dokonać przelewu na poniższe dane:<br /><br />Tytuł przelewu: Zamówienie nr ${data.orderId}<br />Numer konta: 11 2222 4444 7777 8888 9999 1111<br />Szycie to życie<br />Pomorska 32<br />50-216 Wrocław</p><br />
...
exports.statusChanged = (data) => {
    ...
    if(data.status.ID===2) {
        ...
        <p>Jeżeli wybrałeś formę płatności Przelew tradycyjny, to proszę dokonać przelewu na poniższe dane:<br /><br />Tytuł przelewu: Zamówienie nr ${data.orderId}<br />Numer konta: 11 2222 4444 7777 8888 9999 1111<br />Szycie to życie<br />Pomorska 32<br />50-216 Wrocław</p>
...
exports.passwordReminder = (data) => {
    return `<p>Cześć ${data.login}! Kliknij w poniższy link aby zresetować hasło do Twojego konta na sklepie Torebkowa Mania!</p><br />
    <a href=http://localhost:8000/sklep/password/new/customer/${data._id}/${data.token}>http://localhost:8000/sklep/password/new/customer/${data._id}/${data.token}</a><br />
    <p>Link z mozliwością zmiany hasła jest aktywny przez około jedną godzinę.</p>
    <p>Jeżeli to nie Ty, to zignoruj tę wiadomość.</p><br />
    <p>Pozdrawiamy</p><br />
    <p>Zespół Torebkowa Mania</p>`
}